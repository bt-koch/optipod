...
application the measure did not provide early warning compared to officially
...
of the financial sector with the real economy [@ecb2014]. An important 
note is that there is not a single indicator commonly used which
is able to capture the risk within the financial sector. Rather, the
literature has proposed various different indicators. @di2018 provide an
overview of indicators that are used in publications of several institutions
involved in macroprudential analysis and policy, such as the International
...
institutions and in @matros2012 also to the financial sector as a whole
...
the remarks of @capuano2008 can be followed, for which two components are first
...
stock of a firm. A call option grants the holder of the
option the right to buy the underlying asset, here equity stocks, within a given
...
Similarly as in the methodology suggested by @capuano2008 described in
section \@ref(capuanoframework), both $f(V_T)$ and $D$
...
of this project, possible errors may have been introduced to the original code.
